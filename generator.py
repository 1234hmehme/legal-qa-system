...
# --- retriever: phải nằm cùng thư mục, đã code ở bước trước ---
...
- Ở cuối câu trả lời, thêm một dòng riêng:
  "Căn cứ pháp lý: ..." 
  và liệt kê các Điều/Khoản/Điểm tương ứng (không cần trích nguyên văn nội dung luật).
...
def generate_answer(question: str, k: int = 6, base_alpha: float = 0.55) -> Tuple[str, List[str]]:
    ...
    if not context.strip():
        return "Không tìm thấy đủ thông tin trong các văn bản luật đã được lập chỉ mục để trả lời câu hỏi này.", []
    ...
    # 3) Trả về: câu trả lời + danh sách nguồn (để UI hiển thị nếu cần)
    return text, sources


# ===================== QUICK TEST =====================
if __name__ == "__main__":
    q = "Người điều khiển xe mô tô chạy vượt quá tốc độ trên 30 km/h bị phạt bao nhiêu tiền?"
    ans, cites = generate_answer(q, k=5, base_alpha=0.6)
